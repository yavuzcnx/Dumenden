...
export default function Vitrin() {
  ...
  const load = async () => {
    ...
    // 🔧 PGRST201 fix: ilişkiyi adıyla embedle
    const { data: proofs, error: pErr } = await supabase
      ...
      .select(`
        ...
        users!inner ( id, is_plus )
      `)
      .eq('status', 'approved')
      .eq('users.is_plus', false)
      .order('created_at', { ascending: false })
      .limit(200);

    if (pErr) {
      console.error('proofs err', pErr);
      setRows([]);
      setLoading(false);
      return;
    }

    // benim reaksiyonlarım
    const { data: auth } = await supabase.auth.getUser();
    const uid = auth?.user?.id ?? null;
    const { data: mine } = uid
      ? await supabase.from('proof_reactions').select('proof_id,emoji').eq('user_id', uid)
      : { data: [] as any[] };
    const mineMap: Map<string, EmojiKey> = new Map(
      (mine ?? []).map((m: any) => [String(m.proof_id), m.emoji as EmojiKey] as const)
    );
    ...
    const withCounts: Proof[] = await Promise.all(
      (proofs ?? []).map(async (p: any) => {
        ...
        (p.proof_reactions ?? []).forEach((r: any) => {
          ...
          if (e === 'like' || e === 'dislike' || e === 'wow') counts[e] = (counts[e] ?? 0) + 1;
        });
        return {
          id: p.id,
          title: p.title,
  ...
  // 🔁 XP realtime güncelle (opsiyonel ama güzel)
  useEffect(() => {
    let ch: any;
    (async () => {
  ...
  const react = async (proof: Proof, next: EmojiKey) => {
      ...
      prev.map(p =>
        p.id !== proof.id
          ...
          : {
              ...
              counts: {
                ...p.counts,
                ...(prevEmoji ? { [prevEmoji]: Math.max(0, (p.counts[prevEmoji] ?? 1) - 1) } : {}),
                [next]: (p.counts[next] ?? 0) + 1,
              },
            }
      )
    );
    setSelected(sel =>
      sel && sel.id === proof.id
        ? {
            ...
            counts: {
              ...sel.counts,
              ...(prevEmoji ? { [prevEmoji]: Math.max(0, (sel.counts[prevEmoji] ?? 1) - 1) } : {}),
              [next]: (sel.counts[next] ?? 0) + 1,
            },
          }
        : sel
    );
    ...
    const { error } = await supabase
      ...
      .upsert({ proof_id: proof.id, user_id: uid, emoji: next }, { onConflict: 'proof_id,user_id' });
        ...
        prev.map(p =>
          p.id !== proof.id
            ...
            : {
                ...
                counts: {
                  ...p.counts,
                  [next]: Math.max(0, (p.counts[next] ?? 1) - 1),
                  ...(prevEmoji ? { [prevEmoji]: (p.counts[prevEmoji] ?? 0) + 1 } : {}),
                },
              }
        )
      );
      setSelected(sel =>
        sel && sel.id === proof.id
          ? {
              ...
              counts: {
                ...sel.counts,
                [next]: Math.max(0, (sel.counts[next] ?? 1) - 1),
                ...(prevEmoji ? { [prevEmoji]: (sel.counts[prevEmoji] ?? 0) + 1 } : {}),
              },
            }
          : sel
      );
    ...
    () => (
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Text style={{ fontSize: s(28), fontWeight: '900', color: ORANGE, includeFontPadding: false }}>
            Kanıt Vitrini
          </Text>
          <View
            style={{
        ...
        <Text style={{ marginTop: s(6), color: '#6B7280', includeFontPadding: false }}>
          Onaylı kanıtlar — dokun, büyüsün.
        </Text>
      </View>
    ),
    [xp, topPad]
  ...
  const Card = ({ item }: { item: Proof }) => (
    ...
    >
      {!!item.coupons?.title && (
        ...
        >
          <Text style={{ color: ORANGE, fontWeight: '800', includeFontPadding: false }} numberOfLines={1}>
            Kupon: {item.coupons.title}
          </Text>
        </View>
      )}

      {!!item.title && (
        <Text style={{ fontWeight: '900', marginBottom: s(6), textAlign: 'center', includeFontPadding: false }} numberOfLines={2}>
          {item.title}
        </Text>
      )}
      ...
      {/* Emojiler: eşit genişlik + ortalı */}
      <View style={{ flexDirection: 'row', gap: s(6) }}>
        {EMOJIS.map(e => (
          <TouchableOpacity
            key={e.key}
            onPress={() => react(item, e.key)}
            ...
            style={{
              ...
              paddingVertical: s(6),
              borderRadius: s(10),
              backgroundColor: item.my === e.key ? '#FFEEE2' : '#FFF8F3',
              borderWidth: 1,
              borderColor: item.my === e.key ? '#FF9F66' : '#FFE0CC',
              opacity: pending.has(item.id) ? 0.6 : 1,
            }}
          >
            <Text style={{ fontSize: s(16) }}>{e.label}</Text>
            <Text style={{ marginLeft: 6, color: '#444', fontWeight: '700' }}>
              {(item.counts[e.key] ?? 0).toString()}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      <FlatList
        ...
        ListEmptyComponent={
          <View style={{ alignItems: 'center', marginTop: s(40) }}>
            <Text style={{ color: '#6B7280', includeFontPadding: false }}>Henüz kanıt yok.</Text>
          </View>
        }
      />
      ...
      <Modal visible={!!selected} transparent animationType="none" onRequestClose={() => closeAnim(() => setSelected(null))}>
        <Pressable
          onPress={() => closeAnim(() => setSelected(null))}
            ...
            >
              <View style={{ padding: s(12) }}>
                {!!selected.coupons?.title && (
                  ...
                  >
                    <Text style={{ color: ORANGE, fontWeight: '800', includeFontPadding: false }} numberOfLines={1}>
                      Kupon: {selected.coupons.title}
                    </Text>
                  </View>
                )}
                {!!selected.title && (
                  <Text style={{ fontSize: s(18), fontWeight: '900', textAlign: 'center', marginBottom: s(8), includeFontPadding: false }}>
                    {selected.title}
                  </Text>
                )}
              </View>
              ...
              <View style={{ height: s(280), backgroundColor: '#F2F3F5' }}>
                {selected.media_url ? (
                  <Image source={{ uri: selected.media_url }} style={{ width: '100%', height: '100%' }} />
                ) : null}
              </View>

              <View style={{ padding: s(14) }}>
                <Text style={{ color: '#6B7280' }}>{new Date(selected.created_at).toLocaleString()}</Text>
                  ...
                  {EMOJIS.map(e => (
                    <TouchableOpacity
                      ...
                      style={{
                        ...
                        paddingVertical: s(8),
                        borderRadius: s(12),
                        backgroundColor: selected.my === e.key ? '#FFEEE2' : '#FFF8F3',
                        borderWidth: 1,
                        borderColor: selected.my === e.key ? '#FF9F66' : '#FFE0CC',
                        opacity: pending.has(selected.id) ? 0.6 : 1,
                      }}
                    >
                      <Text style={{ fontSize: s(18) }}>{e.label}</Text>
                      <Text style={{ marginLeft: 6, color: '#444', fontWeight: '700' }}>
                        {(selected.counts[e.key] ?? 0).toString()}
                      </Text>
                    </TouchableOpacity>
                  ))}
                </View>
